...
export async function setSessionIndex(sessionId: string, state: SessionState) {
  ...
  let sessionsIndex: string | null = await kv.get(indexKey);
  let index: Record<string, string[]> = sessionsIndex ? JSON.parse(JSON.stringify(sessionsIndex)) : {};
...
export async function setAllQueueTotalJobs(sessionId: string, totalJobs: number) {
  const queues: QueueName[] = ['queryActivities', 'queryShowData', 'crawlRottenTomatoe', 'starSignPicker', 'tvBFF'];
...
export async function getQueueCompletion(sessionId: string, queueName: QueueName): Promise<number> {
  ...
  if (totalJobs === 0) {
    console.log(queueName, "totalJobs > ", totalJobs,  "completedJobs > ", completedJobs )
    return 0;
  }
...
export async function checkIndependentQueuesThresold(sessionId: string): Promise<boolean> {
  let queuesToCheck: QueueName[] = ['tvBFF', 'starSignPicker'];
  let canTrigger = true;
